...
export const DiretorioUsuariosPage: React.FC = () => {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setLoadError(err instanceof Error ? err.message : 'Erro ao carregar o diretório.');
      setUsers([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Diretório de Usuários</h2>
          <p className="text-sm text-slate-500 mt-1">
            Visão em cards para consultar e editar perfis do sistema.
          </p>
        </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center gap-2 py-20 text-slate-400">
          ...
          <span className="text-sm font-medium">Carregando diretório...</span>
        </div>
      ) : loadError ? (
        ...
        <div className="bg-white border border-slate-100 rounded-2xl px-5 py-16 text-center">
          ...
          <p className="text-xs text-slate-400 mt-1">Ajuste os filtros ou a busca global.</p>
        </div>
      ) : (
